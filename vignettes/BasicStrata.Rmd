...
1. It is also highly recommended to use LTASR in conjunction with the tidyverse package. If this has not been installed yet, it will need to be installed for your first use:  
```
...
2.  The next step is to create a data file containing person level information. Each row of the file represents one unique person. The data file must contain the following variables:  

```{r tbl, fig.align='center', echo = FALSE}
kable(per_form)
```

**Note:** R variable names are case sensitive! LTASR assumes all variable names are lower-case.
...
3.	Once the person file has been created, there are several tools that can be used to read it into R. RStudio has a button in the top-right labeled 'Import Dataset' which provides tools for reading in data from various sources. This example will use the read_csv() function to read in a csv file into R (alternatively, other options are excel and SAS datasets from the 'Import Dataset' drop-down). The read_csv() function takes the file path of the person csv file as the first argument.  
...
7.	With your stratified data, you can quickly create a table with all SMRs for each minor found in the rate file using the smr_minor() function as below:  
...
9.	Next, SMRs for various minor groupings can be calculated from this table. One particular grouping are the Majors (as was done in the original LTAS software) defined within the rate file. The smr_major() function can be used for this, which reads in the smr_minor table created in the previous step:  
